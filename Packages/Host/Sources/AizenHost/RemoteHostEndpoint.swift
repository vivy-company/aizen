...
public struct RemoteHostEndpoint: WireEndpoint {
    ...
    private func requirement(for envelope: ProtocolEnvelope) async throws -> Requirement {
        ...
        case ListXcodeDestinationsQueryPayload.identifier:
            let request = try ListXcodeDestinationsQueryPayload(protobufBytes: envelope.payload.protobufBytes)
            let resource = try await requiredResource(request.resourceID)
            return .init(capability: .xcodeRead, spaceID: resource.spaceID, resourceID: resource.id, rateLimitKind: nil)
        ...
        case ReadRepositoryDiffQueryPayload.identifier:
            ...
            let resource = try await requiredResource(request.resourceID)
            return .init(capability: .gitRead, spaceID: resource.spaceID, resourceID: resource.id, rateLimitKind: nil)
        case ReadRepositoryHistoryQueryPayload.identifier:
            let request = try ReadRepositoryHistoryQueryPayload(protobufBytes: envelope.payload.protobufBytes)
            let resource = try await requiredResource(request.resourceID)
            return .init(capability: .gitRead, spaceID: resource.spaceID, resourceID: resource.id, rateLimitKind: nil)
        case ReadRepositoryBranchesQueryPayload.identifier:
            let request = try ReadRepositoryBranchesQueryPayload(protobufBytes: envelope.payload.protobufBytes)
            let resource = try await requiredResource(request.resourceID)
            return .init(capability: .gitRead, spaceID: resource.spaceID, resourceID: resource.id, rateLimitKind: nil)
        case UpdateRepositoryIndexCommandPayload.identifier:
            let request = try UpdateRepositoryIndexCommandPayload(protobufBytes: envelope.payload.protobufBytes)
            let resource = try await requiredResource(request.resourceID)
            return .init(capability: .gitStage, spaceID: resource.spaceID, resourceID: resource.id, rateLimitKind: nil)
        ...
        case PullRepositoryCommandPayload.identifier:
            let request = try PullRepositoryCommandPayload(protobufBytes: envelope.payload.protobufBytes)
            let resource = try await requiredResource(request.resourceID)
            return .init(capability: .gitPull, spaceID: resource.spaceID, resourceID: resource.id, rateLimitKind: nil)
