...
> **路线图** — 1.0.83 是 1.0.x 系列的最终版本。Aizen 即将迎来一次重大的重新构想与产品定位调整。下一个版本将是 2.0.0，且不向后兼容 1.0.x。
...
- **内置浏览器** — 每个窗格独立保存文档、预览、认证流程和本地应用标签页
...
- Xcode 16.0+
- Swift 5.0+
- Zig（用于构建 GhosttyKit）：`brew install zig`

```bash
git clone https://github.com/vivy-company/aizen.git
cd aizen

# 构建 GhosttyKit.xcframework（默认固定到 Vendor/libghostty/VERSION 中的版本）
