...
struct AgentEnvironmentVariablesEditor: View {
    ...
    private func applyPreset(_ preset: EnvironmentVariablePreset) {
    ...
    private func importEnvFile(result: Result<[URL], Error>) {
        importError = nil
    ...
    private func parseEnvFile(_ content: String) -> [AgentEnvironmentVariable] {
        content
            .components(separatedBy: .newlines)
            .compactMap { line -> AgentEnvironmentVariable? in
                let trimmed = line.trimmingCharacters(in: .whitespaces)

                // Skip empty lines, comments, and export-only lines
