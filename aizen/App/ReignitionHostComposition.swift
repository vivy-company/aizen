...
actor ReignitionHostComposition: ReignitionConversationClient {
    ...
    @discardableResult
    func activate() async throws -> CapabilitiesPayload {
    ...
    static func defaultStorageURL(fileManager: FileManager = .default) -> URL {
        ...
        return base
            .appendingPathComponent(hostStorageOwnerIdentifier, isDirectory: true)
    ...
    func discoverXcodeProject(resourceID: ResourceID) async throws -> XcodeProjectDescriptor? {
        try await client.discoverXcodeProject(resourceID: resourceID)
    }
